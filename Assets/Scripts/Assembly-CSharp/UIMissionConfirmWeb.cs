...
public class UIMissionConfirmWeb : MonoBehaviour, TUIHandler
{
	...
	public void Start()
	{
		...
		{
			DataCenter.StateMulti().m_tnet = new TNetObject();
		}
		DataCenter.StateMulti().m_tnet.AddEventListener(TNetEventSystem.CONNECTION, OnConnect);
		DataCenter.StateMulti().m_tnet.AddEventListener(TNetEventSystem.CONNECTION_TIMEOUT, OnConnectTimeout);
		DataCenter.StateMulti().m_tnet.AddEventListener(TNetEventSystem.LOGIN, OnLogin);
	}
	...
	public void HandleEvent(TUIControl control, int eventType, float wparam, float lparam, object data)
	{
		if (control.name == "Left" && eventType == 3)
		{
			...
			m_tui.transform.Find("TUIControl").Find("EquipmentWindow").Find("Equipments")
		...
		{
			if (DataCenter.StateMulti().IsConnect())
			{
				if (DataCenter.StateMulti().IsLogin())
				{
					IntoGame();
				}
				else
				{
					DataCenter.StateMulti().Login();
				}
			}
			else
			{
				DataCenter.StateMulti().Connect();
			}
	...
	{
		UtilUIUpdateAttribute.SetLastScene("MissionConfirmWeb");
	...
	{
		Debug.Log("OnConnect!");
		DataCenter.StateMulti().ConnectSuccess();
		DataCenter.StateMulti().Login();
	}
	...
	{
		DataCenter.StateMulti().LoginSuccess();
		IntoGame();
	}

	private void IntoGame()
	{
		SwitchScene(DataCenter.StateMulti().GetCurrentScene().ToString() + "Web");
		DataCenter.StateMulti().SetCurrentCharactor(m_charactorIndex);
		DataCenter.StateMulti().SetCurrentModelName("Character0" + m_charactorIndex);
	}
}
