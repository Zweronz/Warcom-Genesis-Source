...
public class WeaponPlayerAssaultRifle : WeaponPlayer
{
	...
	public override void Initialize(GameObject prefab, WeaponInfo weaponInfo)
	{
		...
		GameObject original2 = PrefabCache.Load<GameObject>("SoundEvent/weapon_rifle_dryfire");
	...
	protected override void OnDoFire()
	{
		...
		if (Physics.Raycast(ray2, out hitInfo2, num + 0.1f, 4196352))
		{
			...
			WeaponDamageModifierParam weaponDamageModifierParamByType = DataCenter.Conf().GetWeaponDamageModifierParamByType(WeaponType.AssaultRifle);
			float num3 = 1f;
