...
public class WeaponPlayerSniperRifle : WeaponPlayer
{
	...
	public override void Initialize(GameObject prefab, WeaponInfo weaponInfo)
	{
		...
		GameObject original = PrefabCache.Load<GameObject>("SoundEvent/weapon_sniperifle_fire");
		GameObject gameObject = Object.Instantiate(original) as GameObject;
		gameObject.transform.parent = m_transform;
		gameObject.transform.localPosition = Vector3.zero;
		m_evt = gameObject.GetComponent<ITAudioEvent>();
		GameObject original2 = PrefabCache.Load<GameObject>("SoundEvent/weapon_sniperifle_dryfire");
	...
	{
		if (GameManager.Instance().GetCamera().GetCurrentCrosshair()
	...
	protected override void OnDoFire()
	{
		...
		if (Physics.Raycast(ray2, out hitInfo2, num + 0.1f, 4196352))
		{
			...
			WeaponDamageModifierParam weaponDamageModifierParamByType = DataCenter.Conf().GetWeaponDamageModifierParamByType(WeaponType.SniperRifle);
			float num3 = 1f;
