...
public class UtilUIShopArmors : MonoBehaviour
{
	...
	public void Start()
	{
		...
		int num = 0;
		List<ArmorInfo> armorInfo = DataCenter.Conf().GetArmorInfo();
		for (int i = 0; i < armorInfo.Count; i++)
		{
			ArmorInfo armorInfo2 = armorInfo[i];
			GameObject gameObject2 = Object.Instantiate(gameObject) as GameObject;
			gameObject2.name = armorInfo2.name;
			...
			if (armorInfo2.buyMoney == -1)
			{
				component2.frameName = "StatusBarCrystalImg";
				component2.UpdateMesh();
				component3.text = armorInfo2.buyCrystal.ToString("###,###");
			...
			else
			{
				...
				component2.UpdateMesh();
				component3.text = armorInfo2.buyMoney.ToString("###,###");
				component3.UpdateMesh();
			...
			component4.frameName = armorInfo2.icon;
			...
			if (DataCenter.Conf().GetArmorInfoByName(armorInfo2.name).unlockLevel > DataCenter.Save().GetLv())
			{
