...
public class WeaponPlayerHandgun : WeaponPlayer
{
	...
	protected override void OnDoFire()
	{
		...
		if (Physics.Raycast(ray2, out hitInfo2, num + 0.1f, 4196352))
		{
			...
			if (m_characterTransform != null)
			{
				...
				num2 = character2.m_factorHandGunAtk * num2;
				if (character2.m_HPCurrent < character2.m_HPMax * character2.m_factorHpMax * 0.1f)
				{
					hitInfo3.leechKill = true;
				}
			}
			WeaponDamageModifierParam weaponDamageModifierParamByType = DataCenter.Conf().GetWeaponDamageModifierParamByType(WeaponType.Handgun);
			float num3 = 1f;
